...
export DATE            := 20170120
export LOCAL           := no
export PROG_SED        := s,x,x,
export PACKAGE_TARNAME := crosstool-ng
export VERSION         := crosstool-ng-1.22.0-457-gdc3f68c8
export prefix          := /home/arturo/Programmi/ct-ng
...
export INTL_LIBS       :=
export curses_hdr      := ncursesw/curses.h
export gettext         := y

# config options to push down to kconfig
KCONFIG:=  has_static_link=y has_make_3_81_or_newer=y has_xz=y has_lzma has_cvs=y has_svn=y
...
LIB_SUB_DIR := config contrib patches scripts local-patches overlay
